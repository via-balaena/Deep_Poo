...
- Sequence:
  ...
  - Finally, wire Burn inference into the runtime pipeline (`DetectionResult`) to exercise the model end-to-end. ✅ (Burn TinyDet now loads from `checkpoints/tinydet.bin` if present and runs NMS-based detection on live frames.)
