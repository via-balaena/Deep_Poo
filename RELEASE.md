...
Note: Keep all workspace crates on a single shared version for simplicity (e.g., 0.4.0 across the board).
...
- Confirm versions: currently `0.4.0` across all crates.
- Release notes: update the changelog entry in `docs/cortenforge_book/src/00_workspace/changelog.md`.

## Release notes (v0.4.0)
- Removed `cli-support` and `tools` from the `cortenforge` umbrella features.
- Docs updated to reflect direct dependency usage for tools/CLI.
...
## Steps
1) Bump versions (aligned `0.4.0`) and update changelog/release notes if applicable.
2) `cargo fmt --all`
...
7) Tag the repo: `git tag -a v0.4.0 -m "Release v0.4.0"` and push tags: `git push --tags`.
8) Update docs/README with the release notes and any feature/backends notes.

## Verify tags + crates.io
- Tag points at the expected commit: `git show v0.4.0 --no-patch`
- Tag is on origin: `git ls-remote --tags origin v0.4.0`
- Crates.io shows 0.4.0: `https://crates.io/crates/<crate>`
